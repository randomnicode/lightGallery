...
(function() {
    ...
    var ThumbnailWithVideo = function(element) {
    ...
    ThumbnailWithVideo.prototype.init = function() {
        var _this = this;
    ...
    ThumbnailWithVideo.prototype.build = function() {
        var _this = this;
        var thumbList = '';
		...
		function guessMediaType(s) {
			if (typeof(s) == 'undefined') {
				return 'undefined';
			}
			var src = s.toLowerCase();
			var hashIndex = src.lastIndexOf('#');
			var slashIndex = src.lastIndexOf('/');
			
			var dotIndex;
			//deal with hash being part of the filename or timestamp: abc.mp4#t=8.89
			if (hashIndex < slashIndex) { //hash isn't part of the filename or timestamp: abc.mp4#t=8.89
				dotIndex = src.lastIndexOf('.');
			} else {
				dotIndex = src.substring(0, hashIndex).lastIndexOf('.');
			}
			if (dotIndex > -1 && (
			      src.substring(dotIndex).startsWith('.mp4') ||
			      src.substring(dotIndex).startsWith('.webm') ||
			      src.substring(dotIndex).startsWith('.av1')			  
				)) {
				return 'video';
			} else {
				return 'image';
        ...
        function getThumb(src, index, thumb) {
			...
			if (isVideo.youtube && _this.core.s.loadYoutubeThumbnail) {
				thumbhtml='<img src="//img.youtube.com/vi/' + isVideo.youtube[1] + '/' + _this.core.s.youtubeThumbSize + '.jpg" />';
			} else if (isVideo.vimeo && _this.core.s.loadVimeoThumbnail) {
				thumbhtml='<img src="//i.vimeocdn.com/video/error_' + vimeoErrorThumbSize + '.jpg" />';
				vimeoId = 'data-vimeo-id="' + isVideo.vimeo[1] + '"';
    ...
    ThumbnailWithVideo.prototype.setTranslate = function(value) {
    ...
    ThumbnailWithVideo.prototype.animateThumb = function(index) {
    ...
    ThumbnailWithVideo.prototype.enableThumbDrag = function() {
    ...
    ThumbnailWithVideo.prototype.enableThumbSwipe = function() {
        var _this = this;
        var startCoords = 0;
        var endCoords = 0;
        var isMoved = false;
    ...
    ThumbnailWithVideo.prototype.toogle = function() {
        var _this = this;
    ...
    ThumbnailWithVideo.prototype.thumbkeyPress = function() {
        var _this = this;
    ...
    ThumbnailWithVideo.prototype.destroy = function() {
        if (this.core.s.thumbnail && this.core.$items.length > 1) {
    ...
    $.fn.lightGallery.modules.ThumbnailWithVideo = ThumbnailWithVideo;
